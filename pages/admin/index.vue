...
async function fetchItems() {
	...
	try {
		const url = filter.value === 'all'
			? `${apiBase}/admin/submissions`
			: `${apiBase}/admin/submissions?status=${filter.value}`;
...
async function act(id, action) {
	const res = await fetch(`${apiBase}/admin/submissions/${id}/${action}`, {
		method: 'POST',
		headers: { Authorization: authHeader.value },
		credentials: 'include',
	});
...
<style scoped>
.admin-toolbar { display: flex; justify-content: space-between; align-items: center; margin: 0 0 1rem; gap: 1rem; }
.filter-tabs { display: flex; gap: 4px; }
.filter-tabs button {
	padding: 6px 14px; background: #f0f0f0; border: 1px solid #ddd;
	border-radius: 4px; cursor: pointer; font-size: 13px; text-transform: capitalize;
}
.filter-tabs button.active { background: #8CCF7E; color: #111; font-weight: bold; border-color: #7ABF6E; }
.admin-msg { text-align: center; padding: 2rem 1rem; color: #666; }
.admin-msg.error { color: #a33; }
.admin-table { width: 100%; border-collapse: collapse; font-size: 13px; }
.admin-table th, .admin-table td {
	padding: 10px; text-align: left; border-bottom: 1px solid #e5e5e5; vertical-align: top;
}
.admin-table th {
	background: #f7f7f7; font-weight: bold; font-size: 12px;
	text-transform: uppercase; color: #555;
}
.col-thumb { width: 100px; }
.col-thumb img {
	width: 90px; height: 90px; object-fit: contain; background: #000;
